# Export_Cards_As_Text_v2_1_1.py
# Anki add-on for exporting cards from Anki to a text file (CSV, JSON, XML).
# Copyright (c) 2014-2017 Jiri Kriz, http://www.nosco.ch/main/en/contact.php
# License: GNU GPL, version 3 or later; http://www.gnu.org/copyleft/gpl.html

# Version 2.1.1, 28-Aug-2017 (running with Anki 2.1.x)
# (Use the add-on 1589071665 for Anki 2.0.x.)
# Support: None. Use at your own risk. 

# Please notice that I don't get notified when you write a review.
# Please send suggestions and bug reports directly to:
# contact (at) nosco.ch 
...
# Copyright (c) 2014-2017 Jiri Kriz, http://www.nosco.ch/main/en/contact.php
# License: GNU GPL, version 3 or later; http://www.gnu.org/copyleft/gpl.html
# Version 2.1.1, 28-Aug-2017 (running with Anki 2.1)
# (Use the add-on 1589071665 for Anki 2.0.x.)
# Support: None. Use at your own risk. 
# Please notice that I don't get notified when you write a review.
# Please send suggestions and bug reports directly to:
...
# <ul>
# <li><b>Format:</b> Format of the exported text file. One of: CSV (Comma Separated Values), JSON, XML</li>
...
class ExportSettings:
	def __init__(self):
		...
		self.FORMAT_INDEX = FORMAT_INDEX_CSV	  	# 0 = "CSV", 1 = "JSON", 2 = "XML"
...
class Ui_textExportDlg(object):
	def setupUi(self, textExportDlg):
		...
		self.label_3 = QtWidgets.QLabel(textExportDlg)
		self.label_3.setObjectName(_fromUtf8("label_3"))
		self.gridLayout_2.addWidget(self.label_3, 1, 0, 1, 1)
		self.browseButton = QtWidgets.QPushButton(textExportDlg)
		self.browseButton.setObjectName(_fromUtf8("browseButton"))
		self.gridLayout_2.addWidget(self.browseButton, 1, 4, 1, 1)
		self.label_5 = QtWidgets.QLabel(textExportDlg)
		self.label_5.setObjectName(_fromUtf8("label_5"))
		self.gridLayout_2.addWidget(self.label_5, 2, 0, 1, 1)
		self.label_4 = QtWidgets.QLabel(textExportDlg)
		self.label_4.setObjectName(_fromUtf8("label_4"))
		self.gridLayout_2.addWidget(self.label_4, 3, 0, 1, 1)
		self.tagFilterEdit = QtWidgets.QLineEdit(textExportDlg)
		self.tagFilterEdit.setObjectName(_fromUtf8("tagFilterEdit"))
		self.gridLayout_2.addWidget(self.tagFilterEdit, 3, 1, 1, 3)
		self.label_6 = QtWidgets.QLabel(textExportDlg)
		self.label_6.setObjectName(_fromUtf8("label_6"))
		self.gridLayout_2.addWidget(self.label_6, 3, 4, 1, 1)
		self.gridLayout_2.setColumnStretch(1, 1)
		self.verticalLayout.addLayout(self.gridLayout_2)
		self.line = QtWidgets.QFrame(textExportDlg)
		...
		self.allTagsCheck = QtWidgets.QCheckBox(textExportDlg)
		...
		self.deckNameCheck = QtWidgets.QCheckBox(textExportDlg)
		...
		self.buttonBox = QtWidgets.QDialogButtonBox(textExportDlg)
		...
		self.buttonBox.accepted.connect(textExportDlg.accept)
		self.buttonBox.rejected.connect(textExportDlg.reject)
		QtCore.QMetaObject.connectSlotsByName(textExportDlg)


	def retranslateUi(self, textExportDlg):
		textExportDlg.setWindowTitle(_translate("textExportDlg", "Export Cards As Text v2.1", None))
...
class ExportTextDialog(QDialog):
	...
	def adaptFileExtension(self, currentIx):
		parts = os.path.splitext(mw.EXPORT.PATHNAME)
		fileName = parts[0]
		fileExtension = parts[1]
		if currentIx == 0:
			mw.EXPORT.PATHNAME = fileName + ".csv"
		elif currentIx == 1:
			mw.EXPORT.PATHNAME = fileName + ".json"
		else:	
			mw.EXPORT.PATHNAME = fileName + ".xml"
	...
	def fillValues(self):
		...
		self.ui.csvEdit.setEnabled(mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_CSV)
	...
	def formatComboChanged(self, currentIx):
		...
		self.adaptFileExtension(currentIx)
		
	def getPathName(self):
		global FORMAT_INDEX_CSV, FORMAT_INDEX_JSON
		if mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_CSV:
			filter = "CSV/Text files (*.csv *.txt)"
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_JSON:
			filter = "JSON/Text files (*.json *.txt)"
		else:	
			filter = "XML/Text files (*.xml *.txt)"
			
		# See http://doc.qt.io/qt-5/qfiledialog.html#getSaveFileName :
		
		pathName, retFilter = QFileDialog.getSaveFileName(
			mw, 					# parent
			_("Export to File"),	# caption 
			mw.EXPORT.PATHNAME,		# dir 
			filter,
			initialFilter = None,
			options = QFileDialog.DontConfirmOverwrite)
		#pathName = str(pathName)
		mw.EXPORT.PATHNAME = pathName
		return pathName
	...
	def setup(self):
		...
		self.ui.csvEdit.setText("\\t")
		self.ui.formatCombo.activated[int].connect(self.formatComboChanged)
...
class TextExporter(Exporter):
	# see class Exporter in https://github.com/dae/anki/blob/master/anki/exporting.py
	...
	def buildOutputString(self, card, isReversedCard, note):
		global FORMAT_INDEX_CSV, FORMAT_INDEX_JSON, FORMAT_INDEX_XML
		if mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_CSV:
			return self.outputCSV(card, isReversedCard, note)
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_JSON:
			return self.outputJSON(card, isReversedCard, note)
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_XML:
	...
	def writeFooter(self):
		global FORMAT_INDEX_CSV, FORMAT_INDEX_JSON, FORMAT_INDEX_XML
		if mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_JSON: # "JSON"
			line = "{}]\n"  # add empty dummy object, because the previous line ended with ","
			self.file.write(line.encode("utf-8"))
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_XML: # "XML"
			line = "</Anki>\n"
			self.file.write(line.encode("utf-8"))
	...
	def writeHeader(self):
		global FORMAT_INDEX_CSV, FORMAT_INDEX_JSON, FORMAT_INDEX_XML
		line = ""
		if mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_CSV: # "CSV"
			...
			line += "\n"
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_JSON: # "JSON"
			line = "[\n"
		elif mw.EXPORT.FORMAT_INDEX == FORMAT_INDEX_XML: # "XML"
...
# create a new menu item, "ExportCardsAsText"
action = QAction("Export Cards As Text v2.1.1", mw)

# set it to call displayDlg when it's clicked
action.triggered.connect(displayDlg)

# and add it to the tools menu
mw.form.menuTools.addAction(action)

# keep settings over multiple calls of the dialog
mw.EXPORT = ExportSettings() 
